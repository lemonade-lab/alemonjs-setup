...
func ChooseDirectory() (string, error) {
	...
	switch runtime.GOOS {
	case "darwin":
		command = exec.Command("osascript", "-e", "POSIX path of (choose folder with prompt \"选择机器人项目文件夹\")")
	case "windows":
	...
	case "linux":
		command = exec.Command("zenity", "--file-selection", "--directory", "--title=选择机器人项目文件夹")
	default:
