...
import { ArrowRight, Code2, Download, Globe2, LayoutDashboard, Monitor, Send, Smartphone } from 'lucide-react'
...
type ProjectConfig = { template: 'bot' | 'dev'; name: string; destinationMode: 'current' | 'custom'; destination: string; language: string; packageManager: string; eslint: boolean; initializeGit: boolean; usePM2: boolean; imageMode: string; styleMode: string; downloadSkills: boolean; developmentPackages: string[] }
...
const icons: Record<string, ReactNode> = { install: <Download />, manage: <LayoutDashboard />, develop: <Code2 />, deploy: <Send />, desktop: <Monitor />, mobile: <Smartphone />, web: <Globe2 /> }
const capabilityLabels: Record<string, string> = { bubble: '气泡服务', database: '数据存储', discord: 'Discord', onebot: 'OneBot', qqbot: 'QQ Bot' }
...
export default function App() {
  ...
  return <div className="app-shell">
    {guideOpen ? <GuideHome loading={loading} group={guideGroup} goal={selectedID ? activeGoal : undefined} report={report} checking={checking} error={error} creating={creating} creation={creation} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(id ? `/guide/${id}/step/1` : '/guide'); setCreation(null); setError('') }} onClose={() => navigate('/dashboard')} onClearError={() => setError('')} onCheck={checkEnvironment} onCreate={createProject} onFix={setRepairCheck} /> : <Dashboard goals={goals} goal={activeGoal} report={report} checking={checking} error={error} onClearError={() => setError('')} defaultPage={location.pathname.endsWith('/robot') ? 'robot' : 'environment'} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(`/guide/${id}/step/1`); setError('') }} onOpenGuide={openGuide} onCheck={checkEnvironment} onFix={setRepairCheck} />}
    {repairCheck && <EnvironmentFixDialog check={repairCheck} onClose={() => setRepairCheck(null)} />}
  </div>
}

function GuideHome({ loading, group, goal, report, checking, error, creating, creation, onSelect, onClose, onClearError, onCheck, onCreate, onFix }: { loading: boolean; group?: string; goal?: Goal; report: Report | null; checking: boolean; error: string; creating: boolean; creation: Creation | null; onSelect: (id: string | null) => void; onClose: () => void; onClearError: () => void; onCheck: (variant?: string) => void; onCreate: (config: ProjectConfig) => void; onFix: (check: Check) => void }) {
  const backAction = useRef<() => void>(() => {})
  const dispatch = useDispatch()
  const [directoryPickerOpen, setDirectoryPickerOpen] = useState(false)
  useEffect(() => { const open = () => setDirectoryPickerOpen(true); window.addEventListener('alemon:choose-directory', open); return () => window.removeEventListener('alemon:choose-directory', open) }, [])
  const location = useLocation()
  const navigate = useNavigate()
  ...
  return <main className="guide-shell"><section className="guide-window"><GuideHeader onBack={() => group ? navigate('/guide') : backAction.current()} onClose={onClose} showBack={Boolean(goal || group)} />{error && <ErrorNotice message={error} onClose={onClearError} />}{group ? <PurposeGroup group={group} onSelect={onSelect} /> : <FlowView loading={loading} goal={goal} report={report} checking={checking} creating={creating} creation={creation} onSelect={onSelect} onCheck={onCheck} onCreate={onCreate} registerBack={(handler) => { backAction.current = handler }} />}{isCheckStep && missingChecks.length > 0 && <div className="environment-repair">{missingChecks.map((check) => <button key={check.id} onClick={() => onFix(check)}>安装 / 下载 {check.name}</button>)}</div>}<DirectoryPicker open={directoryPickerOpen} multiple={false} onClose={() => setDirectoryPickerOpen(false)} onSelect={(paths) => { dispatch(setProject({ destinationMode: 'custom', destination: paths[0] })); setDirectoryPickerOpen(false) }} /></section></main>
}
...
function FlowView({ loading, goal, report, checking, creating, creation, onSelect, onCheck, onCreate, registerBack }: { loading: boolean; goal?: Goal; report: Report | null; checking: boolean; creating: boolean; creation: Creation | null; onSelect: (id: string | null) => void; onCheck: (variant?: string) => void; onCreate: (config: ProjectConfig) => void; registerBack: (handler: () => void) => void }) {
  ...
  const toggleCapability = (value: string) => dispatch(setDeveloper({ capabilities: capabilities.includes(value) ? capabilities.filter((item) => item !== value) : [...capabilities, value] }))
  const chooseDestination = () => { setFolderError(''); window.dispatchEvent(new Event('alemon:choose-directory')) }
  ...
  const developerPage = () => {
    ...
    switch (flowStep) {
      ...
      case 7: return <><h1>选择开发能力包</h1><p>只安装你现在需要的能力；以后也能在机器人目录的“连接、插件”中单独安装。</p><div className="choice-list">{[['bubble', '气泡服务', '@alemonjs/bubble'], ['database', '数据存储', '@alemonjs/db'], ['discord', 'Discord 连接', '@alemonjs/discord'], ['onebot', 'OneBot 连接', '@alemonjs/onebot'], ['qqbot', 'QQ Bot 连接', '@alemonjs/qq-bot']].map(([value, label, note]) => <button className={capabilities.includes(value) ? 'choice selected' : 'choice'} key={value} onClick={() => toggleCapability(value)}><strong>{label}</strong><small>{note} · {capabilities.includes(value) ? '已选择' : '点击添加'}</small></button>)}</div></>
      case 8: return <><h1>需要做图片功能吗？</h1><div className="choice-list">{[['none', '不需要', '生成文字、按钮和普通消息模板。'], ['html', '纯 HTML', '保留文本帮助模板，后续可按需加入 HTML 渲染方案。'], ['react', 'React / JSX', '安装 jsxp，并生成可拆分组件的图片模板。']].map(([value, label, note]) => <button className={config.image === value ? 'choice selected' : 'choice'} key={value} onClick={() => choose('image', value)}><strong>{label}</strong><small>{note}</small></button>)}</div></>
      case 9: return config.image === 'react' ? choices('图片用什么方式做样式？', [['css', '原生 CSS（推荐）', '最容易理解，不需要再学额外工具。'], ['tailwind', 'Tailwind CSS', '安装 Tailwind 与 CSS 压缩工具。'], ['sass', 'Sass / SCSS', '安装 Sass。'], ['less', 'Less', '安装 Less。']], 'style') : <><h1>不需要样式工具</h1><p>未选择 React / JSX 图片模板，不会安装额外样式依赖。</p></>
      case 10: return <><h1>下载开发技能吗？</h1><p>开发技能像一本 AlemonJS 的使用说明。安装后，Codex 等工具更容易按推荐方式帮你写代码。</p><div className="choice-list"><button className={config.skills === 'yes' ? 'choice selected' : 'choice'} onClick={() => choose('skills', 'yes')}><strong>下载（推荐）</strong><small>下载 alemonjs-dev-skill，后续可随时更新。</small></button><button className={config.skills === 'no' ? 'choice selected' : 'choice'} onClick={() => choose('skills', 'no')}><strong>暂时不下载</strong><small>不会影响机器人运行，以后也可以安装。</small></button></div><a className="download-link" href="https://github.com/lemonade-lab/alemonjs-dev-skill" target="_blank" rel="noreferrer">查看开发技能说明</a></>
      case 11: return <><h1>{creation?.status === 'ready' ? '项目已创建' : '确认创建项目'}</h1>{creation?.status === 'ready' ? <><p>项目已保存至：{creation.path}</p>{creation.path && <a className="primary-button" href={`/dashboard/robot?root=${encodeURIComponent(creation.path)}`}>前往管理机器人</a>}</> : <div className="config-summary"><span>位置：{project.destinationMode === 'current' ? `当前运行目录/${project.name}` : project.destination ? `${project.destination}/${project.name}` : '请返回填写保存位置'}</span><span>语言：{config.language === 'ts' ? 'TypeScript' : 'JavaScript'}</span><span>包管理器：{config.manager}</span><span>开发能力包：{capabilities.length ? capabilities.map((item) => capabilityLabels[item] ?? item).join('、') : '仅基础框架'}</span><span>图片模板：{config.image === 'react' ? `React / JSX · ${config.style}` : config.image === 'html' ? '纯 HTML' : '不生成'}</span><span>代码小助手：{config.eslint === 'yes' ? '启用' : '不启用'}</span><span>项目存档：{config.git === 'yes' ? '初始化 Git' : '跳过'}</span><span>后台运行：{config.pm2 === 'yes' ? '使用 PM2' : '不使用'}</span><span>开发技能：{config.skills === 'yes' ? '下载' : '不下载'}</span></div>}{creation?.logs && <div className="creation-logs">{creation.logs.map((log, index) => <p key={index}>{log}</p>)}</div>}{creation?.status !== 'ready' && <button className="primary-button" onClick={() => onCreate(createConfig())} disabled={creating}>{creating ? '正在创建…' : '确认创建'}</button>}</>
      default: return null
    }
  }
  const installerPage = () => {
    if (flowStep === 0) return <EnvironmentCheckPanel title="检查安装环境" report={report} checking={checking} onCheck={() => onCheck()} />
    if (flowStep === 1) return <><h1>机器人放在哪里？</h1><p>只需给机器人起名并选一个保存位置；其余均使用适合新手的默认设置。</p><div className="project-fields"><label>机器人名称<input value={project.name} onChange={(event) => dispatch(setProject({ name: event.target.value }))} placeholder="my-alemonjs-bot" /></label><div className="location-options"><button className={project.destinationMode === 'current' ? 'choice selected' : 'choice'} onClick={() => dispatch(setProject({ destinationMode: 'current' }))}><strong>当前运行目录（推荐）</strong><small>直接在启动 alemonjs-setup 的文件夹里安装。</small></button><button className={project.destinationMode === 'custom' ? 'choice selected' : 'choice'} onClick={chooseDestination}><strong>选择指定文件夹</strong><small>{project.destinationMode === 'custom' && project.destination ? `已选择：${project.destination}` : '在目录选择器中选择保存位置。'}</small></button></div>{folderError && <ErrorNotice message={folderError} onClose={() => setFolderError('')} />}</div></>
