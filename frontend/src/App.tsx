...
type Release = { tag: string; name: string; url: string; publishedAt: string }
...
export default function App() {
  ...
  return <div className="app-shell">
    {guideOpen ? <GuideHome goals={goals} loading={loading} goal={selectedID ? activeGoal : undefined} report={report} checking={checking} error={error} creating={creating} creation={creation} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(id ? `/guide/${id}/step/0` : '/guide'); setReport(null); setCreation(null); setError('') }} onClose={() => navigate('/dashboard')} onCheck={checkEnvironment} onCreate={createProject} /> : <Dashboard goals={goals} goal={activeGoal} report={report} checking={checking} error={error} defaultPage={location.pathname.endsWith('/robot') ? 'robot' : 'environment'} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(`/guide/${id}/step/0`); setReport(null); setError('') }} onOpenGuide={openGuide} onCheck={checkEnvironment} />}
  </div>
}
...
function FlowView({ goals, loading, goal, report, checking, creating, creation, onSelect, onCheck, onCreate, registerBack }: { goals: Goal[]; loading: boolean; goal?: Goal; report: Report | null; checking: boolean; creating: boolean; creation: Creation | null; onSelect: (id: string | null) => void; onCheck: (variant?: string) => void; onCreate: (config: ProjectConfig) => void; registerBack: (handler: () => void) => void }) {
  ...
  const releaseApp = goal?.id === 'desktop' ? 'alemondesk' : goal?.id === 'mobile' ? 'alemonapp' : goal?.id === 'web' && webEdition === 'clean' ? 'alemongo' : null
  ...
  const downloadSteps = downloadURL ? ['选择镜像', '选择版本'] : ['选择镜像']
  ...
  useEffect(() => { if (!releaseApp) return; request<Release[]>(`/api/v1/releases?app=${releaseApp}`).then((items) => { setReleases(items); setReleaseURL(items[0]?.url ?? null) }).catch(() => { setReleases([]); setReleaseURL(null) }) }, [releaseApp])
  const mirrorURL = (mirror: Mirror, version = releaseURL) => { if (!version) return ''; const index = mirror.url.indexOf('https://github.com'); return index === -1 ? version : mirror.url.slice(0, index) + version }
  const releasePicker = () => <label className="release-picker">选择版本<select value={releaseURL ?? ''} onChange={(event) => { setReleaseURL(event.target.value); setDownloadURL(selectedMirror ? mirrorURL(selectedMirror, event.target.value) : event.target.value) }}>{releases.map((item) => <option value={item.url} key={item.tag}>{item.tag} · {item.name}</option>)}</select>{!releases.length && <small>正在获取正式版本列表…</small>}</label>
  const developerPage = () => {
  ...
  const installerPage = () => {
    if (flowStep === 0) return <><h1>检查安装环境</h1><p>{checking || !report ? '正在检查 Node.js 与 Git…' : report.ready ? '检查通过。确认后继续下一步。' : '发现需要处理的环境问题。'}</p>{report ? <div className="compact-checks">{report.checks.map((check) => <span className={check.status} key={check.id}>{check.status === 'ready' ? '✓' : '!'} {check.name}：{check.detail}</span>)}</div> : <span className="checking-indicator" />}{report && !report.ready && <button className="primary-button" onClick={() => onCheck()} disabled={checking}>重新检查</button>}</>
    if (flowStep === 1) return <><h1>机器人放在哪里？</h1><p>只需给机器人起名并选一个保存位置；其余均使用适合新手的默认设置。</p><div className="project-fields"><label>机器人名称<input value={project.name} onChange={(event) => dispatch(setProject({ name: event.target.value }))} placeholder="my-alemonjs-bot" /></label><div className="location-options"><button className={project.destinationMode === 'current' ? 'choice selected' : 'choice'} onClick={() => dispatch(setProject({ destinationMode: 'current' }))}><strong>当前运行目录（推荐）</strong><small>直接在启动 alemonjs-setup 的文件夹里安装。</small></button><button className={project.destinationMode === 'custom' ? 'choice selected' : 'choice'} onClick={() => dispatch(setProject({ destinationMode: 'custom' }))}><strong>指定文件夹</strong><small>在你输入的完整本机路径中安装。</small></button></div>{project.destinationMode === 'custom' && <label>保存位置<input value={project.destination} onChange={(event) => dispatch(setProject({ destination: event.target.value }))} placeholder="例如 /Users/你的用户名/Desktop" /></label>}</div></>
  ...
  const webPage = () => {
    ...
    return <><h1>{webEdition === 'docker' ? 'Docker Compose 快速启动' : '下载 AlemonGo'}</h1><p>{webEdition === 'docker' ? '下一步会生成 docker-compose.yml 并启动服务。' : '选择一个正式版本，再打开对应的下载页。'}</p>{webEdition === 'clean' && releasePicker()}{webEdition === 'clean' && releaseURL && <a className="primary-button" href={releaseURL} target="_blank" rel="noreferrer">打开所选版本下载页</a>}</>
  }
  const buildPage = () => {
  ...
  const downloadPage = () => {
    if (flowStep === 0) return <><h1>选择下载镜像</h1><p>选择一个下载来源后继续；下一页再选要下载的版本。</p><div className="choice-list">{goal?.mirrors?.map((mirror) => <button className={selectedMirror?.url === mirror.url ? 'choice selected' : 'choice'} key={mirror.url} onClick={() => { setSelectedMirror(mirror); setDownloadURL(mirror.url) }}><strong>{mirror.name}</strong><small>{mirror.name === 'GitHub 官方' ? '从 GitHub 官方下载' : '下载速度可能更快'}</small></button>)}</div></>
    return <><h1>选择下载版本</h1><p>默认已选择最新正式版本。确认后点击右下角下载。</p>{releasePicker()}<p className="selected-download">下载镜像：{selectedMirror?.name ?? 'GitHub 官方'}</p></>
  }
