...
export default function App() {
  ...
  return <div className="app-shell">
    {guideOpen ? <GuideHome goals={goals} loading={loading} goal={selectedID ? activeGoal : undefined} report={report} checking={checking} error={error} creating={creating} creation={creation} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(id ? `/guide/${id}/step/1` : '/guide'); setReport(null); setCreation(null); setError('') }} onClose={() => navigate('/dashboard')} onCheck={checkEnvironment} onCreate={createProject} onFix={setRepairCheck} /> : <Dashboard goals={goals} goal={activeGoal} report={report} checking={checking} error={error} defaultPage={location.pathname.endsWith('/robot') ? 'robot' : 'environment'} onSelect={(id) => { if (id === 'manage') { navigate('/dashboard/robot'); return }; navigate(`/guide/${id}/step/1`); setReport(null); setError('') }} onOpenGuide={openGuide} onCheck={checkEnvironment} />}
    {repairCheck && <EnvironmentFixDialog check={repairCheck} onClose={() => setRepairCheck(null)} />}
  </div>
}

function GuideHome({ goals, loading, goal, report, checking, error, creating, creation, onSelect, onClose, onCheck, onCreate, onFix }: { goals: Goal[]; loading: boolean; goal?: Goal; report: Report | null; checking: boolean; error: string; creating: boolean; creation: Creation | null; onSelect: (id: string | null) => void; onClose: () => void; onCheck: (variant?: string) => void; onCreate: (config: ProjectConfig) => void; onFix: (check: Check) => void }) {
...
function FlowView({ goals, loading, goal, report, checking, creating, creation, onSelect, onCheck, onCreate, registerBack }: { goals: Goal[]; loading: boolean; goal?: Goal; report: Report | null; checking: boolean; creating: boolean; creation: Creation | null; onSelect: (id: string | null) => void; onCheck: (variant?: string) => void; onCreate: (config: ProjectConfig) => void; registerBack: (handler: () => void) => void }) {
  ...
  useEffect(() => { if (!releaseApp) return; request<Release[]>(`/api/v1/releases?app=${releaseApp}`).then((items) => { setReleases(items); setReleaseURL(items[0]?.url ?? null); setSelectedAssetURL(null) }).catch(() => { setReleases([]); setReleaseURL(null); setSelectedAssetURL(null) }) }, [releaseApp])
