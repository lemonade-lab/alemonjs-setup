...
export default function App() {
  ...
  async function checkEnvironment(variant?: string) { const selectedVariant = typeof variant === 'string' ? variant : ''; try { setChecking(true); setError(''); setReport(await request<Report>('/api/v1/checks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ goalId: activeID, variant: selectedVariant }) })) } catch (reason) { setError(reason instanceof Error ? reason.message : '环境检查未完成，请稍后重试。') } finally { setChecking(false) } }
