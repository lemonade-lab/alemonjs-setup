...
export type WorkspaceWebViewTab = { key: string; root: string; entryID: string; package: string; title: string; openedAt: string; lastActiveAt: string }
type WorkspaceState = { projects: WorkspaceProject[]; activeProjectID: string; drafts: Record<string, string>; developerMode: boolean; webviewTabs: WorkspaceWebViewTab[]; activeWebviewTabKey: string }

const initialState: WorkspaceState = { projects: [], activeProjectID: '', drafts: {}, developerMode: false, webviewTabs: [], activeWebviewTabKey: '' }
const workspaceSlice = createSlice({
  ...
  reducers: {
    ...
    selectProject(state, action: PayloadAction<string>) { state.activeProjectID = action.payload; state.activeWebviewTabKey = ''; const root = state.projects.find(item => item.id === action.payload)?.path; if (!root) return; const recent = state.webviewTabs.filter(item => item.root === root).sort((left, right) => right.lastActiveAt.localeCompare(left.lastActiveAt))[0]; state.activeWebviewTabKey = recent?.key ?? '' },
    ...
    openWebviewTab(state, action: PayloadAction<Omit<WorkspaceWebViewTab, 'openedAt' | 'lastActiveAt'>>) { const now = new Date().toISOString(); const existing = state.webviewTabs.find(item => item.key === action.payload.key); if (existing) existing.lastActiveAt = now; else state.webviewTabs.push({ ...action.payload, openedAt: now, lastActiveAt: now }); state.activeWebviewTabKey = action.payload.key },
    activateWebviewTab(state, action: PayloadAction<string>) { const tab = state.webviewTabs.find(item => item.key === action.payload); if (!tab) return; tab.lastActiveAt = new Date().toISOString(); state.activeWebviewTabKey = tab.key },
    clearActiveWebviewTab(state) { state.activeWebviewTabKey = '' },
    closeWebviewTab(state, action: PayloadAction<string>) { const index = state.webviewTabs.findIndex(item => item.key === action.payload); if (index < 0) return; const active = state.activeWebviewTabKey === action.payload; state.webviewTabs.splice(index, 1); if (active) state.activeWebviewTabKey = state.webviewTabs[index]?.key ?? state.webviewTabs[index - 1]?.key ?? '' },
    pruneWebviewTabs(state, action: PayloadAction<{ root: string; entryIDs: string[] }>) { state.webviewTabs = state.webviewTabs.filter(item => item.root !== action.payload.root || action.payload.entryIDs.includes(item.entryID)); if (!state.webviewTabs.some(item => item.key === state.activeWebviewTabKey)) state.activeWebviewTabKey = '' },
...
export const persistedWorkspace = persistReducer({
  ...
  storage,
  whitelist: ['projects', 'activeProjectID', 'drafts', 'developerMode', 'webviewTabs', 'activeWebviewTabKey'],
  transforms: [privateDraftsTransform]
}, workspaceSlice.reducer)
export const { addProjects, selectProject, removeProject, setDraft, clearDraft, setDeveloperMode, openWebviewTab, activateWebviewTab, clearActiveWebviewTab, closeWebviewTab, pruneWebviewTabs } = workspaceSlice.actions
