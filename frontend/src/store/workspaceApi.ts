...
export type GitWorkspace = { root: string; repository: boolean; gitRoot?: string; remote?: string; branch?: string; upstream?: string; ahead: number; behind: number; changes: Array<{ status: string; path: string }>; branches: Array<{ name: string; current: boolean; upstream?: string }>; remoteBranches: Array<{ name: string; remote: string; branch: string }>; commits: Array<{ sha: string; shortSha: string; subject: string; createdAt: string }>; tags: Array<{ name: string; subject?: string; createdAt?: string }>; remotes: Array<{ name: string; url: string }> }
...
export const workspaceApi = createApi({
  ...
  endpoints: (build) => ({
    ...
    gitWorkspace: build.query<GitWorkspace, { root: string; view: 'commit' | 'history' | 'tag' | 'branch' | 'remote' }>({ query: ({ root, view }) => `robot/git?${new URLSearchParams({ root, view })}`, providesTags: (_result, _error, arg) => [{ type: 'GitStatus', id: arg.root }] }),
    ...
    gitWorkspaceAction: build.mutation<RobotResult, { root: string; action: 'fetch' | 'pull' | 'push' | 'commit' | 'branch-create' | 'branch-switch' | 'branch-track' | 'branch-delete' | 'tag-create' | 'tag-push' | 'tag-delete' | 'remote-add' | 'remote-set-url' | 'remote-remove'; value?: string; message?: string }>({ query: (body) => ({ url: 'robot/git', method: 'POST', body }), invalidatesTags: (_result, _error, body) => [{ type: 'GitStatus', id: body.root }] }),
