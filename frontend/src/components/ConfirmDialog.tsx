...
export function ConfirmDialog({ open, title, message, confirmLabel = '确认继续', cancelLabel = '取消', subtitle = '此操作会修改当前机器人项目或其运行状态。', busy, destructive = false, onCancel, onConfirm }: Props) {
  ...
  return createPortal(<div className="app-dialog-backdrop" role="presentation" onMouseDown={onCancel}>
    <section className="app-dialog" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <p>{message}</p>
      <footer><button className="secondary-button" onClick={onCancel}>{cancelLabel}</button><button className={destructive ? 'danger-button' : 'primary-button'} disabled={busy} onClick={onConfirm}>{busy ? '处理中…' : confirmLabel}</button></footer>
    </section>
  </div>, document.body)
}
