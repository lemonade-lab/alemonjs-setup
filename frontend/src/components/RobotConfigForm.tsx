...
type Props = { busy: boolean; toolbar?: ReactNode; onSave: (content: string) => void }
...
export function RobotConfigForm({ busy, toolbar, onSave }: Props) {
  ...
  const group = (name: string, count: number, children: ReactNode, open = false) => <details className="config-section" open={open}><summary><strong>{name}</strong><span>{count} 项</span></summary><div className="form-grid">{children}</div></details>
  return <section className="config-form official-config-form"><header className="config-form-header">{toolbar}<button className="primary-button" disabled={busy} onClick={() => onSave(toYaml(values))}>保存</button></header>{group('基础配置', 6, <>{field('port', 'CBP 端口', '17117')}{field('serverPort', '应用端口', '18110')}{field('input', '应用入口', 'lib/index.js')}{field('login', '登录平台', 'discord')}{field('url', 'CBP 地址', 'ws://127.0.0.1:17117')}<label>全量接收<select value={values.fullReceive} onChange={(event) => set('fullReceive', event.target.value)}><option value="">不设置</option><option value="true">开启</option><option value="false">关闭</option></select></label></>, true)}{group('权限与机器人标识', 4, <>{field('masterID', '主人 ID', '多个用逗号分隔')}{field('masterKey', '主人 Key', '多个用逗号分隔')}{field('botID', '机器人 ID', '多个用逗号分隔')}{field('botKey', '机器人 Key', '多个用逗号分隔')}</>)}{group('消息控制', 8, <>{field('disabledRegular', '禁用文本正则', '/闭关')}{field('disabledSelects', '禁用事件', 'message.create, private.message.create')}{field('disabledUserID', '禁用用户 ID', '多个用逗号分隔')}{field('disabledUserKey', '禁用用户 Key', '多个用逗号分隔')}{field('redirectRegular', '重定向正则', '^#')}{field('redirectTarget', '重定向目标', '/')}{field('mappingRegular', '映射匹配文本', '/帮助')}{field('mappingTarget', '映射替换文本', '/help')}</>)}{group('处理器与模块', 3, <>{field('repeatedEventTime', '重复事件窗口（毫秒）', '60000')}{field('repeatedUserTime', '重复用户窗口（毫秒）', '1000')}{field('apps', '启用模块', 'alemonjs-openai, alemonjs-xianyu')}</>)}{group('CBP 连接', 7, <>{field('cbpTimeout', '连接超时（毫秒）', '180000')}{field('cbpReconnect', '重连间隔（毫秒）', '6000')}{field('cbpHeartbeat', '连接心跳（毫秒）', '18000')}{field('cbpHealthCheck', '健康检查（毫秒）', '30000')}{field('cbpUserAgent', 'User-Agent', 'platform')}{field('cbpDeviceID', '设备 ID', 'auto-generated')}<label>CBP 全量接收<select value={values.cbpFullReceive} onChange={(event) => set('cbpFullReceive', event.target.value)}><option value="">不设置</option><option value="1">开启</option><option value="0">关闭</option></select></label></>)}</section>
}
