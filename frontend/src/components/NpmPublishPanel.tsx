...
export function NpmPublishPanel({ root, busy, onRun }: Props) {
  ...
  return <section className="npm-publish-panel">
    ...
    <section className="publish-status-line" aria-label="发布状态"><span>本地 <b>{status.localVersion || '未设置'}</b></span><span>npm <b>{status.published ? status.latestVersion : '从未发布'}</b></span><span>账户 <b className={status.loggedIn ? 'ready' : 'warning'}>{status.loggedIn ? status.username : '未登录'}</b></span></section>
		<section className="publish-source"><div><strong>发布源码</strong><small>npm 会从这个已提交版本打包，不会包含尚未提交的文件。</small></div><label>{status.branch || '当前分支'}<select value={sourceCommit} onChange={(event) => { setSourceCommit(event.target.value); setPreview(null); setConfirming(false) }} disabled={!sourceCommits.length}>{sourceCommits.length ? sourceCommits.map((item) => <option key={item.sha} value={item.sha}>{item.shortSha} · {item.subject} · {item.createdAt}</option>) : <option value="">暂无可选提交</option>}</select></label></section>
