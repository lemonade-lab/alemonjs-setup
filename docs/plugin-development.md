...
- `advanced: true` 把操作标记为低频/专业能力；管理台会把它收进默认收起的「高级操作」折叠区，让常见操作保持简洁。可选，默认 `false`。
...
- 字段的 `help`（可选）是一句通俗说明，会显示在输入框下方，用于向新手解释术语（如 CIDR、MTU）。
...
管理台会把 `output` 渲染成结构化结果卡片：每行若以 `✓ `、`! ` 或 `? ` 开头（符号后跟一个空格），会按「正常 / 失败 / 警告」分别着色；其余行（含以空格或制表符开头的行）按普通文本原样显示。建议用于诊断类输出，例如：

```text
✓ 系统：darwin/arm64
接口 en0（up，MTU 1500，MAC 72:5e:5c:d0:15:11）
  192.168.1.4/24
✓ DNS：registry.npmjs.org 解析正常 → 104.16.0.0
! HTTPS 连通性：registry.npmjs.org:443 连接失败
? 暂无端口转发规则
```

- 前缀符号后必须是**一个空格**；`✓x`、`!tab` 等会被当作普通行。
- 行首前导空格不会被去除，因此缩进列表天然是普通行。
- 若插件确实需要在行首输出这些符号，用反斜杠转义：`\✓ `、`\! `、`\? ` 会去掉反斜杠并按普通行渲染。
- 整段输出不含任何前缀行时，管理台回退到纯文本渲染，与未加前缀完全一致。
